...
#Contribution:
#Xuyi Shi: Totally about 34% contribution 
##Pre-processing data and check it, Perfection of function "next.word", Modify the matrix M, Write the generating sentences code and testing code

#Chi Zhang: Totally about 34% contribution, 
##Build the M matrix and token, Check the code, Write the main code of "next.word" function, Brainstorm for generating sentences and modify

#Jiachen Guang: Totally about 32%
#Building vocabulary and word frequency statistics,Establish a single starting word function and the final realization function, Check the final code
...
#identical(a, a_original) 
#To make sure there exists changes after doing "gsub"
#setdiff(a,a_original)
#show the different between a and a_original
...
##--------------Q5:Building vocabulary and word frequency statistics------------
...
##------------------------Q6:Build the M matrix, token--------------------------
a_token <- match(a,b) 
# generate the vector contains the tokens for representing the whole text
...
M_4 <- create_sequence_matrix(a, a_token, mlag = 4) 
# We can try to change the "mlag" to be 5 or 6 and see what's going on. 
M <- M_4
#sequence matrix with mlag = 4

##-------------------Q7:Write the function of next words------------------------
...
start.token <- function(b, M1, word = NULL) {
  # b: Vocabulary vector
...
#----------Complete testing of Question 8 and Question 9, under mlag = 4--------
# Method 1: Random selection:
set.seed(0)
start_random <- start.token(b, a_token)

# Method 2: Specify interesting vocabulary
start_romeo <- start.token(b, a_token, "romeo")


# Generate sentences

sentences_markov <- list() # create a list to save the sentences

# Sentence 1 (random start)
sentence1 <- make_sentence(start_random, M, a_token, b)
sentences_markov[["Random start"]] <- sentence1$sentence

# Sentence2 (starting word 'romeo')
sentence2 <- make_sentence(start_romeo, M, a_token, b)
sentences_markov[["starting word 'romeo'"]] <- sentence2$sentence

# Print the results of three different starting words
cat("Generated sentence:\n");print(sentences_markov)
