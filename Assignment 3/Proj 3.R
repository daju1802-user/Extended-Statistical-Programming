...
#Contribution:
#Xuyi Shi(35%): Create the function(test_gradient and calc_bic), doing the test and searching the optimal lambda by comparing the BIC
#Figure out the log-likelihood function and confirm, Merged the equations for pnll and pnll_grad, including weights with teammates.
#Check the matrices function, change the BIC to be better

#Jiachen Guang(35%): Build the matrices function(such as X,X_tilde) and modify the function.Add some comments and the belief of code.
#Identify and report any problems encountered. Comment the final plot.

#Chi Zhang(30%): Doing the bootstrap and the final plot, adding some comments and checking the final file including the comments.
...
# ============================================================================
# Create model matrices X_tilde, X, and penalty matrix S
# ============================================================================
...
# ============================================================================
# Likelihood and gradient functions, doing finite differencing test
# ============================================================================
...
# ============================================================================
# Fit model with lambda = 5e-5 and plot results
# ============================================================================
...
# ============================================================================
# Grid search for optimal lambda using BIC
# ============================================================================
...
for (b in 1:n_bootstrap) {
  ...
  fit_bootstrap <- optim(par = gamma_current,fn = pnll,gr = pnll_grad,
                         y = y, X = X, S = S, lambda = lambda_opt,w = wb,
                         method = "BFGS",control = list(maxit = 1000))
...
plot_range_x <- range(c(t, t_infection))
...
polygon(c(t_infection, rev(t_infection)),
        c(f_lower, rev(f_upper)),
        col = rgb(0, 0, 1, 0.2), border = NA)
lines(t_infection, f_hat_opt, col = "blue", lwd = 2)
...
cat("Peak infection time:", t_infection[which.max(f_hat_opt)], "\n")
cat("Peak death rate:", round(max(mu_opt), 2), "\n")
cat("Peak death time:",t[which.max(mu_opt)],"\n")
...
#The plot visualizes the relationship between the observed daily deaths from COVID-19 
#and the estimated underlying daily infection counts inferred through the smooth deconvolution model.
#On the left y-axis, the black dots represent the observed daily deaths reported by hospitals, 
#while the red curve shows the fitted death counts predicted by the model.
#On the right y-axis, the blue curve depicts the estimated daily infections, representing the reconstructed infection trajectory 
#after adjusting for the infection-to-death delay distribution.  
#The shaded blue band shows the 95% confidence interval. This interval reflects the uncertainty in the estimated infection curve 
#due to sampling variation and model fitting.
